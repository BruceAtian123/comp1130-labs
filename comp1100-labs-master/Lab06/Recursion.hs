...
forgotBaseCase n | n > 0 = n * forgotBaseCase (n-1)
forgotBaseCase n | n < 0 = undefined
...
factorial2 n = accumulate 1 n
  where
    ...
    accumulate a 0 = a
    accumulate a n = accumulate (a*n) (n-1)
...
invert x = case x of
    ...
    x:xs -> inverse (x:xs) []
       where
            inverse :: [a] -> [a] -> [a]
            inverse (x:xs) ys = inverse xs (x:ys)
            inverse [] x = x
...
rucksack an target = [x | x<-findSubList an, sum x == target]

findSubList :: [Integer] -> [[Integer]]
findSubList [] = [[]]
findSubList (x:xs) = (isInList x an)++an
                    where
                    an = findSubList xs

isInList :: Integer -> [[Integer]] -> [[Integer]]
isInList _ [] = []
isInList a (x:xs) = (a:x) : isInList a xs